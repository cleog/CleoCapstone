...
export default function Home(parms) {
    ...
    const deleteMatchHandler = (answerIndex, matchIndex) => {
        ...
        return <main className="flex min-h-screen flex-col items-center font-mono p-24 bg-white">
            <button className="bg-orange p-1 text-black w-30 h-15 self-start"><a href="/admin"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
                    ...
                    <li className="py-2 outline p-4 outline-orange/50 bg-orange/20 divide-y divide-dashed divide-black mt-4">
                        {question.answerOptions.map((a, ai) => (
                            <div className="py-4" key={a.field_id}>
                                ...
                                <button onClick={() => addMatchHandler(ai)} className="bg-orange p-1 ml-4 text-black text-m" type="button">Add</button>
                            </div>
                        ))}
                        <div className="py-8">
                            <span>
                                <button onClick={addAnswerHandler} className="bg-orange text-black" type="button">Add Answer</button>
                                <button className="ml-8 bg-orange text-black" onClick={hasNoErrors ? handleSubmitHandler : handleSubmitErrorHandler} type="button" enabled={inputErrors && inputErrors.length == 0}>
                                    Submit
                                </button>
                            </span>
                        </div>
